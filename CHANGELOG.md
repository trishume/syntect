...
- Add `ThemeScopeRenderer` (`syntect::easy`) for theme-aware rendering via a user-supplied formatting closure [#627]
...
- Add `ThemeHighlight` (`syntect::easy`) for theme-based line-by-line highlighting of strings, returning `Vec<(Style, &str)>` — the high-level replacement for the old `HighlightLines` use case [#627]
- `HighlightFile` restructured to wrap `ThemeHighlight` with a buffered file reader, with a public `highlight` field and `highlight_line` convenience method [#627]
