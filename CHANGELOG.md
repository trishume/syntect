...
- Add `ThemeScopeRenderer` (`syntect::easy`) for theme-aware rendering with style merging (adjacent same-style tokens coalesced), defaulting to ANSI terminal output via `AnsiStyleWriter` [#627]
- Add `StyleWriter` trait (`syntect::easy`) for format-specific styled text output (`open`/`close`/`text`), with built-in `AnsiStyleWriter` [#627]
- Add `InlineHtmlStyleWriter` (`syntect::html`) for inline-styled `<span style="...">` HTML output via `StyleWriter` [#627]
...
- `highlighted_html_for_string`/`highlighted_html_for_file` reworked to use `HighlightLines` with `InlineHtmlStyleWriter` [#627]
