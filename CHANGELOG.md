...
- Add `syntect::io` module [#627]
- Add `HighlightedWriter<'a, R: ScopeRenderer, W: io::Write>` (`syntect::io`), a streaming highlighter that implements `std::io::Write` and drives parsing + pluggable rendering with branch-point backtracking support, generic over the output sink (`Vec<u8>` by default, or any `io::Write` for streaming) [#627]
- Add `ScopeRenderer` trait (`syntect::io`) for pluggable scope-based rendering with pre-resolved atom strings [#627]
- Add `render_line()` core rendering loop that drives a `ScopeRenderer` [#627]
- Add `ThemedANSIScopeRenderer` (`syntect::io`) for theme-aware ANSI terminal rendering with style merging (adjacent same-style tokens coalesced) [#627]
- Add `ClassedHTMLScopeRenderer` (`syntect::html`) for `<span class="...">` output with CSS classes derived from scope atoms [#627]
- Add `ThemedHTMLScopeRenderer` (`syntect::html`) for theme-aware inline-styled `<span style="...">` HTML output with style merging [#627]
- Add `Scope::with_atom_strs()` closure-based API for reading atom strings without direct repository access [#627]
- `ClassedHTMLGenerator` preserved as backward-compatible wrapper around `HighlightedWriter` [#627]
- `highlighted_html_for_string`/`highlighted_html_for_file` reworked to use `HighlightedWriter` with `ThemedHTMLScopeRenderer` [#627]
